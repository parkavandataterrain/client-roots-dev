...
function ReactTable({ columns, data, top }) {
  ...
  return (
    <Stack>
      {top && (
        <Box sx={{ p: 2 }}>
          <TablePagination gotoPage={gotoPage}  style={{color: "#1A1F25"}} rows={rows} setPageSize={setPageSize} pageIndex={pageIndex} pageSize={pageSize} />
        </Box>
      )}
      ...
      <Table {...getTableProps()}>
        ...
        <TableBody {...getTableBodyProps()}>
          {page.map((row) => {
            ...
            return (
              <TableRow {...row.getRowProps()}>
                {row.cells.map((cell) => (
                  <TableCell {...cell.getCellProps([{ className }])}>{cell.render('Cell')}</TableCell>
                ))}
              </TableRow>
            );
          })}
          ...
          {!top && (
            <TableRow  style={{color: "#1A1F25"}}>
              <TableCell sx={{ p: 2 }} style={{color: "#1A1F25"}} colSpan={7}>
                <TablePagination style={{color: "#1A1F25"}} gotoPage={gotoPage} rows={rows} setPageSize={setPageSize} pageIndex={pageIndex} pageSize={pageSize} />
              </TableCell>
            </TableRow>
          )}
        </TableBody>
