...
export default function DepartmentTable() {
  ...
  const deleteRecord = (id) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`/api/resources/department/${id}`)
          .then((response) => {
            fetchData();
            notifySuccess(`${actionPerformed}d Successfully`);
          })
  ...
  const actionColumn = !(
    isAllowed("change_departments") || isAllowed("delete_departments")
  )
    ? []
    : [
        {
            ...
            return (
              <>
                <div className="h-100 w-100 flex flex-row gap-2 justify-center items-center">
                  <PrivateComponent permission="change_departments">
                    <button
                  ...
                  <PrivateComponent permission="delete_departments">
                    <button
                      className="p-1 hover:bg-teal-400 bg-opacity-50 hover:rounded"
                      title="Delete"
                      onClick={(e) => {
  ...
  return (
    <Box sx={{ height: 400, width: "100%" }}>
      <Paper sx={{ width: "100%" }}>
        <PrivateComponent permission="view_departments">
          {" "}
...
function AddNewDepartment({
  ...
  const onSubmit = (data) => {
    ...
    let endpoint = "/api/resources/department";
    let axiosCall = axios.post;

    if (isUpdate) {
      endpoint = `/api/resources/department/${id}`;
    ...
    axiosCall(`${endpoint}`, postData, {
      ...
      .then((response) => {
        Swal.fire({
          title: "Success!",
          text: `Department ${isUpdate ? "Updated" : "Created"}`,
          icon: "success",
          timer: 2000,
        });
        refresh && refresh();
      ...
      .catch((error) => {
        ...
        Swal.fire({
          title: "Error!",
          text: `Unable to ${isUpdate ? "Update" : "Create"} Department`,
          icon: "error",
          timer: 2000,
        });
      })
        ...
        <Modal.Title className="m-0 p-0 w-100">
          <div className="flex justify-between items-center w-100">
            <span className="text-white text-base">
              {`${isUpdate ? "Update" : "Add New"} Department`}
            </span>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="p-4 w-100">
            <div className="mb-4">
              ...
              <input
                ...
                style={{
                  border: `1px solid ${
                    !errors.department_name ? "#5BC4BF" : "red"
                  }`,
                  fontSize: "14px",
                }}
  ...
  return (
    <div className="grid grid-cols-4 justify-between gap-2 w-100 mt-1 mb-4">
      ...
      <div className="col-end-5 col-span-2 flex gap-2 items-center justify-end">
        ...
        <PrivateComponent permission="add_departments">
          <div className="ms-3">
            <button
